...
스마트팩토리와 협동로봇을 소재로 한 자기소개 반응형 웹페이지.
...
- 프로젝트 구역은 두 덩이다. 위는 주요 프로젝트 고정 카드, 아래는 전체 저장소 목록이다.
- 주요 프로젝트는 `js/main.js`의 `PINNED` 배열에 이름을 올린 저장소만 그린다(현재 `smart-factory-sim`, `air-sensor`, `chzzk-vod-toolbar`, `url-shortner`). 카드 문구·주제 태그는 저장소 정보에서 그대로 꺼내므로 GitHub와 어긋나지 않는다. 고정한 저장소는 아래 목록에서 빠진다.
- 전체 저장소 목록은 GitHub API(`/users/sarguments/repos`)에서 최근 수정순 최대 100개를 받는다(기본 호출은 30개까지만 오므로 `per_page=100`을 붙였다). 목록은 `REPO_LIMIT`(9개)까지만 보여 주고 나머지는 `전체 보기` 버튼으로 펼친다. 언어 필터는 고정 저장소를 뺀 목록에서 만든다.
- `index.html` — 페이지 뼈대
...
- 메뉴·스크롤: 햄버거 토글(CSS 아이콘 ☰→✕ + 스크린리더용 숨김 라벨 '메뉴', `aria-expanded` 동기화), `IntersectionObserver` 등장(임계값 [0, 0.2] — 구역이 화면보다 길면 진입 즉시 표시), 300px에서 맨 위로 버튼, 60px에서 내비 표시 변경.
...
- 기준값: 스크롤탑 300px·내비 변경 60px·등장 임계값 [0, 0.2]. 바꾸면 이 줄을 고친다.
...
- 임계값 [0, 0.2]: 0.2는 감시 대상이 화면에 20% 들어오면 실행한다는 뜻이고, 0은 닿는 순간을 함께 잡는다. 카드가 한 열로 쌓이는 모바일처럼 구역 높이가 화면보다 훨씬 크면 20% 도달 자체가 불가능해 0.2만으로는 콜백이 오지 않는다. 한 번 보이면 감시를 그만둔다(`unobserve`).
