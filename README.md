...
스마트팩토리와 협동로봇을 소재로 한 자기소개 반응형 웹페이지. 외부 라이브러리 없이 HTML·CSS·JavaScript만으로 만든다.

소개 문구와 기술 목록은 로봇팔·설비 데이터를 다루는 방향으로 적었다. 프로젝트 카드는 GitHub API가 각 저장소의 설명을 그대로 그리므로, 저장소 설명이 곧 카드 문구가 된다.
